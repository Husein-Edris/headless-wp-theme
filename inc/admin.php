...
class HeadlessProAdmin {
        ...
        <div class="wrap">
            ...
            <div class="headless-admin-grid">
                <div class="headless-card">
                    <h2>📊 API Status</h2>
                    <p>Monitor your GraphQL and REST API endpoints.</p>
                    <a href="<?php echo admin_url('admin.php?page=headless-api-status'); ?>" class="button button-primary">View API Status</a>
                </div>
                
                <div class="headless-card">
                    <h2>🔧 Content Management</h2>
                    <p>Manage your custom post types and content structure.</p>
                    <a href="<?php echo admin_url('admin.php?page=headless-content'); ?>" class="button button-primary">Manage Content</a>
                </div>
                ...
                <table class="widefat">
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td><?php echo class_exists('WPGraphQL\WPGraphQL') ? '✅ Active' : '❌ Inactive'; ?></td>
                            <td><a href="<?php echo home_url('/graphql'); ?>" target="_blank" class="button button-small">Test</a></td>
                        </tr>
                        <tr>
                            <td>REST API</td>
                        ...
                        <tr>
                            <td>GraphiQL IDE</td>
                            <td><code><?php echo admin_url('admin.php?page=graphiql-ide'); ?></code></td>
                            <td><?php echo class_exists('WPGraphQL\WPGraphQL') ? '✅ Available' : '❌ Not Available'; ?></td>
                            <td>
                                <?php if (class_exists('WPGraphQL\WPGraphQL')): ?>
                                    <a href="<?php echo admin_url('admin.php?page=graphiql-ide'); ?>" class="button button-small">Open IDE</a>
                                <?php endif; ?>
                            </td>
                        </tr>
                    </tbody>
    ...
    private function render_api_status() {
        $apis = array(
            'GraphQL' => class_exists('WPGraphQL\WPGraphQL'),
            'ACF' => class_exists('ACF'),
            'ACF GraphQL' => function_exists('acf_get_setting'),
            'REST API' => true,
        );
        ...
        foreach ($apis as $name => $status) {
            ...
            echo '<div class="headless-card ' . $status_class . '">';
            echo '<h3>' . $status_emoji . ' ' . $name . '</h3>';
            echo '<p>Status: <strong>' . $status_text . '</strong></p>';
            echo '</div>';
        }
    }
    ...
    public function enqueue_admin_scripts($hook) {
        if (strpos($hook, 'headless') !== false) {
            wp_enqueue_style('headless-admin', HEADLESS_THEME_URL . '/assets/admin.css', array(), HEADLESS_THEME_VERSION);
            wp_enqueue_script('headless-admin', HEADLESS_THEME_URL . '/assets/admin.js', array('jquery'), HEADLESS_THEME_VERSION, true);
        }
    }
    ...
    public function add_admin_bar_links($wp_admin_bar) {
        ...
        $wp_admin_bar->add_node(array(
            'id' => 'headless-frontend',
            'title' => '🌐 View Frontend',
            'href' => get_theme_mod('frontend_url', home_url()),
    ...
    public function dashboard_widget_status() {
        ...
        <div class="headless-dashboard-widget">
            ...
            <p><strong>GraphQL:</strong> <?php echo class_exists('WPGraphQL\WPGraphQL') ? '✅ Active' : '❌ Install WPGraphQL'; ?></p>
            <p><strong>ACF:</strong> <?php echo class_exists('ACF') ? '✅ Active' : '❌ Install ACF'; ?></p>
            ...
            <div style="margin-top: 15px;">
                <a href="<?php echo admin_url('admin.php?page=headless-pro'); ?>" class="button button-primary">Manage Settings</a>
                <?php if (class_exists('WPGraphQL\WPGraphQL')): ?>
                    <a href="<?php echo admin_url('admin.php?page=graphiql-ide'); ?>" class="button">GraphiQL IDE</a>
                <?php endif; ?>
            </div>
        </div>
        <?php
    ...
    public function show_headless_notices() {
        ...
        if (!empty($missing_plugins)) {
            ...
            echo '<p><strong>Headless Pro:</strong> For optimal functionality, please install: ' . implode(', ', $missing_plugins) . '</p>';
            echo '</div>';
        }
